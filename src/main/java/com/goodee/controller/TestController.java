...
public class TestController {
	...
	public void download(HttpServletResponse response) throws Exception{
		String path = "D:/sample/01.JSP 기초.pdf"; //다운받을 파일명
		
		// 다운로드 받고자 하는 파일에 대한 Path 지정
		Path file = Paths.get(path); 
		
		// 파일이름 utf-8로 인코딩 : 파일 이름이 깨지지 않도록 설정하기 위한
		String filename = URLEncoder.encode(file.getFileName().toString(), "UTF-8");
		
		// response 객체의 헤더 세팅(이 헤더가 없으면 데이터를 제대로 받아올 수 없습니다.)
		response.setHeader("Content-Disposition", "attachment;filename="+filename);
		
		// 파일 channel 설정 (우리가 받아오고자하는 파일을 READ 읽을 수 있게 받아옵니다. FileChannel을 단순히 가져오기 위한 채널로 쓸 것입니다.)
		FileChannel fc = FileChannel.open(file, StandardOpenOption.READ);
		
		// response에서 output스트림 추출 (getOutputStream 파일 io에서 가져온 단방향 스트림입니다. 
		//리스폰스는 이렇게 두번 코드를 칠 수 밖에 없는게 채널을 바로 못뽑아내서 아웃풋스트림만 뽑을 수 있습니다.)
		//그래서 그 밑에 채널을 한번 더 추출하는 것입니다.
		OutputStream out = response.getOutputStream();
		
		// outputStream에서 channel 추출 ( response 아웃풋 스트림 밑 채널 추출)
